...
## Creating the Interact Active Ability
...
![Plugin installation using the Epic Games Launcher](_static/img/active/21a_settings_active.png)
...
Open the `GASActorBase` and override `Get Interaction Context` and make a struct based 
on `TBIA_InteractionContext`.

![Plugin installation using the Epic Games Launcher](_static/img/active/27_interaction_context.png)

Open `Event Graph` and let's override `On Pre Interact`, `On Post Interact`, `On Cancel Interact` 
