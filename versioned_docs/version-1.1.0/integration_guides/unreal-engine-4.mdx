...
This integration guide will show you how to integrate *AGRPRO* with *TwistedBytes Interaction System*, so that
you can use both in combination in your own projects.

:::info
This guide is meant to be a general integration guide that you can use as reference when integrating plugins from unreal engine marketplace which
are mainly `NON-GAS` based plugins.
:::
...
Next, navigate to `<Games>/<Project Name>/Source/<Project Name>.Build.cs` and open the file <Step text="1"/>.
...
You can do that by copying the code <Step text="4"/> as shown in the image below.
...
And under that we should override the function `GetAbilitySystemComponent` which is needed to make `ASC` works correctly, and create the constructor function
to initialize the components.

Copy the code as in <Step text="5"/>
...
Under that we want to define the constructor function to create the components we declared in the header file. You 
can do that by adding the code shown in <Step text="3"/>.
...
Then we want to define the overridden function `GetAbilitySystemComponent` as shown below <Step text="4"/>.
