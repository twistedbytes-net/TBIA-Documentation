...
- Input Action: `IA_InteractActive` <Step text="2"/>
- Input Tag: `TBIA.Input.Interaction.InteractActive` <Step text="3"/> (See [this](/docs/getting-started/migration-guide#add-gameplay-tags) if this tag does not exist in your project)
- Gameplay Ability: `InteractActive` <Step text="4"/> (See [this](/docs/quick-start/using-active) if you haven't created it yet)
